...
function push_changes_to_feature_branch() {
  ...
  if git show-ref --quiet refs/heads/feature/sg-app-dashboard; then
    echo "Branch feature/sg-app-dashboard already exists. Checking out the branch..."
    git checkout feature/sg-app-dashboard
  else
    echo "Branch feature/sg-app-dashboard does not exist. Creating new branch..."
    git branch feature/sg-app-dashboard
    git checkout feature/sg-app-dashboard
  fi

  git push -f --set-upstream "${product_repo_path}" "feature/sg-app-dashboard"
...
handle_policy_action() {
  ...
  case "${action}" in
    ...
    "IMPORT-FROM-GIT-TO-NEWRELIC")
      local account_id="$(jq .account_id ${product_repo_path}/products/${product_name}/config/${normalised_environment}.json)"
      local app_name="$(jq .appName ${product_repo_path}/products/${product_name}/config/${normalised_environment}.json)"
      app_name=$(echo ${app_name} | tr '[:lower:]' '[:upper:]')
      local policy_name="SG Alert Policy - $(echo ${app_name} | tr -d '\"') (${normalised_environment})"
